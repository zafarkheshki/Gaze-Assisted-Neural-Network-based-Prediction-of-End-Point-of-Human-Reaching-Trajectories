...
# Input data
with open("/home/zafar/catkin_ws/src/Thesis/reading_angle_values/New_Data/traj_pred_plus_gaze.csv") as fp:
    reader = csv.reader(fp, delimiter=",", quotechar='"')
    # next(reader, None)  # skip the headers
    data_read = [row for row in reader]

input_data = np.concatenate(data_read)
input_data = np.reshape(input_data,(330,6)).astype('float64')
...
torch.save(model.state_dict(), os.path.join(os.getcwd(),'traj_gaze_ee_model.pt'))
